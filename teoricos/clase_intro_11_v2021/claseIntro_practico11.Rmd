...
```{r, echo = FALSE, eval = T, message=FALSE, width = "50%"}
library(knitr)
knitr::include_graphics('../../imgs/gradiente_reproducibilidad.png')
...
```{r, echo = FALSE, eval = T, message=FALSE, width = "50%"}
library(knitr)
knitr::include_graphics('../../imgs/bioconductor_logo_rgb.jpg')
```
...
## Componentes de una función 
El *ambiente* de una funcion controla el modo en que R encuentra el valor asociado a un nombre.
...
#<-> el indentado no es obligatorio, pero ayuda a leer  
  ...
  return(una_variable_nueva) # atencion: a veces una funcion no devuelve una variable
...
eleva_y_resta(y = 2) # nos juega una mala pasada el ambiente de la funcion...
```
...
eleva_y_resta = function(x = 1, y =1){
  ...
  if(!is.numeric(x) || !is.numeric(y)){ # en condicionales como if usar '||' o '&&'
...
- El practico de hoy esta en [este link](http://www.higiene.edu.uy/ddbp/lbc/html/Practico9.html) (clickear).
- passwd: **HfqsRNA2_**